							...
							if (timeSinceCheck > ONE_WEEK_IN_MS) {
								try {
									...
									if (
										...
										modDetails.url.startsWith(
											'https://www.nexusmods.com',
										)
									) {
										const modNexusId = modDetails.url
											.split('/')
											.pop();
										if (
											typeof modNexusId === 'string' &&
											modNexusId.length > 0
										) {
											const nexusModDetail =
												await api.nexus_mod_details({
													game_domain_name:
														selectedGame!
															.nexus_slug,
													mod_id: Number(modNexusId),
												});
											updatedModVersionData[
												vi
											].last_time_checked = Date.now();
											if (
												mv.latest_version !==
